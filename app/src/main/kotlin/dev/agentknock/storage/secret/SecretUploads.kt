...
internal class SecretUploads(
    ...
    suspend fun describeEnvironmentSecretUpload(
        ...
        val existingVariables =
            existing
                ?.let { secret ->
                    snapshot.variablesBySecret[secret.id]
                        .orEmpty()
                        .associateBy(EnvironmentVariableEntity::name)
                }
                .orEmpty()
        ...
        upload.variables.toSortedMap().forEach { (name, value) ->
            ...
            if (current == null) {
                ...
                val same =
                    (material.decryptEnvironmentValue(current) as? DecryptionResult.Plaintext)
                        ?.value
        ...
        val removed =
            if (upload.mode == SecretUploadMode.REPLACE) {
        ...
        return EnvironmentSecretUploadResult.Valid(
            EnvironmentSecretUploadSummary(
                ...
                variableSensitivity =
                    upload.variables.keys.associateWith { name ->
    ...
    suspend fun prepareEnvironmentSecretUpload(
        ...
        runCatching { validateSecretName(approvedName) }
            .exceptionOrNull()
            ?.message
            ?.let {
                return EnvironmentSecretUploadPreparation.Invalid(it)
        ...
        val existing =
            when (upload.mode) {
                ...
                SecretUploadMode.UPDATE -> {
                    ...
                    snapshot.secrets
                        .singleOrNull { it.id == expected.secretId }
                        ?.takeIf {
                            it.name == upload.name &&
                                it.revision == expected.revision &&
        ...
        val existingVariables =
            existing
                ...
                .orEmpty()
        val variables =
            upload.variables.map { (name, value) ->
                val current = existingVariables[name]
                val id = current?.id ?: newId()
                val sensitive = upload.variableSensitivity[name] ?: current?.sensitive ?: true
                val encrypted =
                    material.encryptEnvironmentValue(id, secretId, name, sensitive, value)
    ...
    suspend fun applyPreparedEnvironmentSecretUpload(
        ...
        val applied =
            dao.applyEnvironmentUploadIfCurrent(
    ...
    private fun validateUploadInput(upload: EnvironmentSecretUpload): String? = runCatching {
        validateSecretName(upload.name)
