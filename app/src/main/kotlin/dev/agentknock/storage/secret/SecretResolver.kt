...
private class ParsedSecretSnapshot(snapshot: SecretSnapshot) {
    ...
    val variablesBySecret: Map<String, List<EnvironmentVariableEntity>> = snapshot.variablesBySecret
    val sshKeysBySecret: Map<String, ParsedSshKey> =
        snapshot.sshKeys
            .mapNotNull { key ->
            ...
            .toMap()
    private val secretTypesById: Map<String, SecretType> =
        snapshot.secrets
            .mapNotNull { secret ->
...
internal class SecretResolver(
                ...
                SecretType.ENVIRONMENT ->
                    SecretMetadata(
                        ...
                        environmentVariableNames =
                            snapshot.variablesBySecret[secret.id]
                                .orEmpty()
                                .map(EnvironmentVariableEntity::name),
                    )
                SecretType.SSH ->
                    snapshot.sshKeysBySecret[secret.id]?.let { key ->
                        SecretMetadata(
                            name = secret.name,
                            description = secret.description,
    ...
    suspend fun resolve(
        ...
        return SecretResolution(
            ...
            nonSensitiveEnvironmentValues =
                if (includeValues && values is RequestedSecretsResult.Available) {
    ...
    suspend fun signGitMessage(
        ...
        sign(secretName, expectedPublicKey) { key ->
    ...
    private suspend fun <T> sign(
        ...
        val currentPublic = runCatching {
            material.publicKey(key.entity, key.algorithm)
        }
            .getOrElse {
                return SignatureResult.SecretCorrupted
            }
        val expectedPublic = runCatching {
        ...
        val plaintext =
            when (val decrypted = material.decryptSshKey(key.entity, key.algorithm)) {
    ...
    private inner class Selection(
        ...
        private val secretByName =
            requestedNames.mapNotNull(snapshot.secretsByName::get).associateBy(SecretEntity::name)

        private val selectedVariablesBySecretId =
            secretByName.values.associate { secret ->
                secret.id to
        ...
        fun description(): RequestedSecretDescription =
            RequestedSecretDescription(
                secrets =
                    requestedNames.mapNotNull { name ->
                        secretByName[name]?.metadata(selectedVariables(secretByName.getValue(name)))
                    },
                reviewMetadata =
                    requestedNames.mapNotNull { name ->
                        secretByName[name]?.reviewMetadata(
                            selectedVariables(secretByName.getValue(name))
                        )
                    },
                missingSecrets = requestedNames.filterNot(secretByName::containsKey),
                containsSensitiveMaterial =
                    secretByName.values.any { secret ->
        ...
        suspend fun values(includeValues: Boolean): RequestedSecretsResult {
            ...
            val typed =
                secretByName.values.associateWith {
                    snapshot.typeOf(it) ?: return RequestedSecretsResult.UnsupportedSecretType
                }
            if (typed.values.count { it == SecretType.SSH } > 1) {
                return RequestedSecretsResult.MultipleSshKeys
            }
            typed.entries
                .firstOrNull { (secret, type) ->
                    type == SecretType.SSH && selections[secret.name] != null
                }
                ?.let {
                    return RequestedSecretsResult.EnvironmentOptionsForSshSecret(it.key.name)
                }
            ...
            for ((secret, type) in typed) {
                ...
                val required =
                    selection.only.orEmpty() +
                        selection.rename.keys +
            ...
            for ((secret, type) in typed) {
                ...
                for (variable in selectedVariables(secret)) {
                    val value =
                        when (val decrypted = material.decryptEnvironmentValue(variable)) {
            ...
            for (name in requestedNames) {
                ...
                values[name] =
                    when (typed.getValue(secret)) {
                        ...
                        SecretType.SSH -> {
                            val key =
                                snapshot.sshKeysBySecret[secret.id]
                                    ?: return RequestedSecretsResult.SecretCorrupted
        ...
        private fun SecretEntity.reviewMetadata(
            ...
            return when (snapshot.typeOf(this)) {
                ...
                SecretType.SSH ->
                    snapshot.sshKeysBySecret[id]?.let {
                        SecretReviewMetadata(
                            id = id,
                            revision = revision,
                            name = name,
        ...
        suspend fun nonSensitiveEnvironmentValues(
            ...
            requestedNames
                .mapNotNull { name ->
                    ...
                    val delivered =
                        (available[name] as? SecretValues.Environment)?.environment.orEmpty()
                    val values =
                        snapshot.variablesBySecret[secret.id]
                            .orEmpty()
                            ...
                            .mapNotNull { variable ->
                                delivered[variable.name]?.let { variable.name to it }
                                    ?: optionalEnvironmentValue(variable)?.let {
                                        variable.name to it
                                    }
                            }
                            .toMap(linkedMapOf())
        ...
        private suspend fun optionalEnvironmentValue(variable: EnvironmentVariableEntity): String? {
            val decrypted =
            ...
            return when (decrypted) {
                ...
                DecryptionResult.KeyUnavailable,
                DecryptionResult.AuthenticationFailed,
                DecryptionResult.UnsupportedFormat -> null
            }
        }
        ...
        private fun SecretEntity.destination(
            ...
            selected: Boolean,
        ): EnvironmentVariableReviewDestination =
            when {
