...
internal val MIGRATION_2_3 =
    Migration(2, 3) { connection ->
        connection.execSQL(
            "UPDATE secrets SET approval_mode = 'ask_me' WHERE approval_mode = 'temporary'"
        )
        connection.execSQL(
...
private fun removeEnvironmentVariableNotes(connection: SQLiteConnection) {
    ...
    connection.execSQL("ALTER TABLE environment_variables_new RENAME TO environment_variables")
    connection.execSQL(
...
private fun migrateAuditEvents(connection: SQLiteConnection) {
    ...
    connection.execSQL("CREATE INDEX index_audit_events_occurred_at ON audit_events (occurred_at)")
}
