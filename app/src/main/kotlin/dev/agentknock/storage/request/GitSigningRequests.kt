...
internal const val GIT_SIGN_POLICY_DENIAL_MESSAGE = "Approval settings denied use of the SSH key."
private const val GIT_SIGN_COMPLETION_ABORTED_DETAIL = "Git signing was aborted by the client."
...
internal class GitSigningRequests(
    ...
    suspend fun processIncoming(
        ...
        val contents =
            runCatching { gitSignProtocol.decodeRequest(plaintext) }.getOrNull() ?: return null
        ...
        suspend fun finishReview(
            ...
            if (approvalSettingsDenied) {
                denial = InvocationDenialReason.POLICY_DENIED to GIT_SIGN_POLICY_DENIAL_MESSAGE
            }
    ...
    suspend fun approve(
        ...
        val invocation =
            approvedParentInvocation(request) ?: return RequestDecisionResult.ParentUnavailable
        ...
        return writeTransaction.execute {
            ...
            val currentRequest =
                dao.getRequestById(request.id)
                    ?: return@execute ConditionalRequestUpdate.UNAVAILABLE
            val currentGitSign =
                dao.getGitSignRequest(request.id)
                    ?: return@execute ConditionalRequestUpdate.UNAVAILABLE
            if (
    ...
    private suspend fun persistDecision(
        ...
        return writeTransaction.execute {
            ...
            val currentGitSign =
                dao.getGitSignRequest(request.id) ?: return@execute RequestDecisionResult.NotFound
            if (
    ...
    private suspend fun refreshApprovalEvaluation(
        ...
        val currentGitSign =
            dao.getGitSignRequest(requestId) ?: return@execute RequestDecisionResult.NotFound
        if (
            currentRequest.state != InboxRequestState.ACTION_REQUIRED.storedName ||
                currentGitSign.decision != null
        ) {
    ...
    private suspend fun requestAiReview(
        ...
        val invocationSecrets =
            invocation.providedSecretsJson?.let { stored ->
        ...
        AuditRecord(
            ...
            data =
                request.requestAuditData() +
                    gitSign.copy(decision = decision.storedName).auditData(),
        )

    private fun GitSignRequestEntity.auditData() =
        auditDataOf(
