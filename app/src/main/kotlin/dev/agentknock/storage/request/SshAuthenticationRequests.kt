...
internal const val SSH_AUTHENTICATION_DENIAL_MESSAGE = "SSH authentication denied on device."
...
internal class SshAuthenticationRequests(
    ...
    suspend fun processIncoming(
        ...
        suspend fun finishReview(
            ...
            val aiReview =
                if (aiInputsChanged) {
                    AiReview(
                        ...
                        explanation =
                            "The client, SSH key, instructions, or approval settings changed during AI review.",
                    )
                } else {
                    reviewResult?.review
                }
            val evaluation =
                (if (aiInputsChanged) currentEvaluation else initialEvaluation)?.copy(
                    aiReview = aiReview
                )
            val authorization =
            ...
            if (clientUnavailable && denial == null) {
                denial = InvocationDenialReason.OTHER to "The paired client is no longer available."
            }
            if (invocationUnavailable && denial == null) {
                denial =
                    InvocationDenialReason.OTHER to "The parent invocation is no longer available."
            }
            ...
            if (approvalSettingsDenied) {
                denial =
                    InvocationDenialReason.POLICY_DENIED to SSH_AUTHENTICATION_POLICY_DENIAL_MESSAGE
            }
            val aiDenied = denial == null && aiReview?.decision == AiReviewDecision.DENY
            if (aiDenied) {
                denial = InvocationDenialReason.POLICY_DENIED to checkNotNull(aiReview.explanation)
            }
            val shouldApprove =
                !aiInputsChanged &&
                    denial == null &&
            ...
            val automaticDecisionSource =
                automaticApproval.decisionSource
            ...
            val automaticDecisionAudit = automaticDecision?.let {
                AuditRecord(
                    ...
                    detail =
                        if (aiReview != null && aiReview.decision != AiReviewDecision.ASK_USER) {
                ...
                launchAiReview(
                    ...
                    initialRequest.requestJson,
                    {
                        requestAiReview(
                            ...
                            parentElapsedSeconds =
                                if (now >= invocationRequest.receivedAt) {
    ...
    suspend fun approve(
        ...
        val authentication =
            dao.getSshAuthenticationRequest(requestId) ?: return RequestDecisionResult.NotFound
        ...
        val invocation =
            invocationSnapshot(request) ?: return RequestDecisionResult.ParentUnavailable
        ...
        val storedEvaluation =
            authentication.approvalEvaluationJson?.let(::decodeApprovalEvaluation)
        if (
            storedEvaluation == null || !storedEvaluation.hasSameSecretPolicies(currentEvaluation)
        ) {
            return refreshApprovalEvaluation(requestId, json.encodeToString(currentEvaluation))
        }
        val expectedPublicKey =
            expectedPublicKey(invocation, authentication.secretName)
        ...
        val response =
            sealResponse(request, protocol.approvedResponse(signature))
                ?: return RequestDecisionResult.ClientUnavailable
        return persistDecision(
            request = request,
            authentication = authentication,
    ...
    suspend fun finishAiReview(
        ...
        return writeTransaction.execute {
            ...
            val currentRequest =
                dao.getRequestById(request.id)
                    ?: return@execute ConditionalRequestUpdate.UNAVAILABLE
            val currentAuthentication =
                dao.getSshAuthenticationRequest(request.id)
                    ?: return@execute ConditionalRequestUpdate.UNAVAILABLE
            if (
    ...
    suspend fun complete(
        ...
        return writeTransaction.execute {
            ...
            val authentication = dao.getSshAuthenticationRequest(request.id) ?: return@execute false
    ...
    private suspend fun persistDecision(
        ...
        require(grant == null || decision == ApprovalDecision.APPROVED) {
            "Only approved SSH authentication can grant temporary access"
        }
        return writeTransaction.execute {
            val now = currentTimeMillis()
            val currentRequest =
                dao.getRequestById(request.id) ?: return@execute RequestDecisionResult.NotFound
            val currentAuthentication =
                dao.getSshAuthenticationRequest(request.id)
            ...
            when (result) {
                ...
                ConditionalRequestUpdate.ACTION_REQUIRED,
                ConditionalRequestUpdate.UNAVAILABLE -> RequestDecisionResult.ApprovalChanged
            }
        }
    }
    ...
    private suspend fun refreshApprovalEvaluation(
        ...
        val currentRequest =
            dao.getRequestById(requestId) ?: return@execute RequestDecisionResult.NotFound
        val authentication =
            dao.getSshAuthenticationRequest(requestId)
                ?: return@execute RequestDecisionResult.NotFound
    ...
    private suspend fun requestAiReview(
                ...
                ?: return AiReviewAttempt(
                    review =
                        AiReview(
                            decision = AiReviewDecision.ASK_USER,
                            explanation =
                                "The relative timing of the parent invocation is unavailable.",
                        ),
                    request = null,
                )
        val invocationSecrets =
            invocation.providedSecretsJson?.let {
        ...
        val request =
            approvalReviewSshAuthenticationRequest(
                client = client,
                secretName = contents.secret,
    ...
    private fun InboxRequestEntity.isPending(
        ...
        state == InboxRequestState.ACTION_REQUIRED.storedName && authentication.decision == null
        ...
        AuditRecord(
            ...
            data =
                request.requestAuditData() +
                    authentication.copy(decision = decision.storedName).auditData(),
        )

    private fun SshAuthenticationRequestEntity.auditData() =
        auditDataOf(
