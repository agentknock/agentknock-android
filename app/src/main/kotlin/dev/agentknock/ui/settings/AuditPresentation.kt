...
internal fun AuditEvent.presentation(): AuditPresentation =
    when (type) {
        ...
        AuditEventType.SECRET_USE_DECIDED ->
            secretUseEvent(
                title =
                    decisionTitle(
                        "Secret use",
                        nonSensitiveTitle = "Non-sensitive data provided automatically",
                    ),
        ...
        AuditEventType.SECRET_LIST_COMPLETED ->
            AuditPresentation(
                title =
                    if (outcome == AuditOutcome.COMPLETED) {
        ...
        AuditEventType.SECRET_UPLOAD_RECEIVED ->
            AuditPresentation(
                title =
                    if (outcome == AuditOutcome.RECEIVED) {
        ...
        AuditEventType.ENVIRONMENT_VARIABLE_ADDED ->
            environmentVariableEvent("Environment variable added")
        AuditEventType.ENVIRONMENT_VARIABLE_UPDATED ->
            environmentVariableEvent("Environment variable updated")
        AuditEventType.ENVIRONMENT_VARIABLE_DELETED ->
            environmentVariableEvent("Environment variable deleted")
...
private fun AuditEvent.environmentVariableEvent(title: String) =
    AuditPresentation(
        title,
...
private fun AuditEvent.deviceEvent(title: String) = AuditPresentation(title, AuditCategory.DEVICE)

private fun AuditEvent.pairingAddressEvent(title: String) =
    AuditPresentation(title, AuditCategory.DEVICE, "Pairing address")

private fun AuditEvent.aiReviewTitle(operation: String): String =
    when (outcome) {
        ...
        AuditOutcome.APPROVED ->
            when (decisionSource) {
                ...
                AuditDecisionSource.NON_SENSITIVE ->
                    nonSensitiveTitle ?: "$operation approved automatically"
                AuditDecisionSource.VALIDATION,
                null -> "$operation approved"
            }
        AuditOutcome.DENIED ->
            when (decisionSource) {
...
private fun AuditEvent.decisionDetailLabel(): String? =
    when {
...
private fun AuditEvent.completionDetailLabel(): String? = if (detail != null) "Reason" else null

private fun AuditEvent.invalidOutcome(): Nothing =
    error("Unexpected outcome $outcome for audit event ${type.code}")

internal fun AuditDecisionSource.displayName(): String =
    when (this) {
