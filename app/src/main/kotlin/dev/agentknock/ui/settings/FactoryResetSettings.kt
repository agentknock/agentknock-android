...
internal fun FactoryResetSettings(
    ...
    LaunchedEffect(state) {
        if (state == FactoryResetUiState.ClearFailed) {
            consumeClearFailure()
            report(
                "Android could not clear Agentknock's app data. Clear its storage from Android settings."
            )
        }
    }
    ...
    Column(modifier) {
        ...
        Column(
            ...
            Text(
                "Setup starts again with a new device identity and pairing address. Every client must pair again."
            )
            Text(
                "This does not cancel a Google Play subscription. Manage subscriptions in Google Play."
            )
            Text(
