...
class ApprovalPresentationTest {
    @Test
    fun pendingEscalation_isNotPresentedAsAnAiApproval() {
    ...
    @Test
    fun aiResultsAndFailures_remainDistinct() {
        ...
        assertEquals(
            "AI review inactive · Ask me",
            aiReviewLabel(AiReview(failure = AiReviewFailure.SUBSCRIPTION_REQUIRED)),
        )
        assertEquals(
            "AI review unavailable",
            aiReviewLabel(AiReview(failure = AiReviewFailure.UNAVAILABLE)),
        )
        assertNull(approvalSummary(null, null, null))
    }

    @Test
    fun deterministicAndTemporaryDecisions_doNotImplyAiReview() {
        assertEquals(
            "Approval settings",
            approvalSummary(null, ApprovalDecision.APPROVED, "policy"),
        )
        assertEquals(
            "Temporary access",
            approvalSummary(null, ApprovalDecision.APPROVED, "temporary_access"),
        )
        assertEquals(
            "No approval needed",
            approvalSummary(null, ApprovalDecision.APPROVED, "non_sensitive"),
        )
    }
}
