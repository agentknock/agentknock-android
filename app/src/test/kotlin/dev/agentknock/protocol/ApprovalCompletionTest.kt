        ...
        decoders.forEach { decode ->
            listOf("APPROVED", "DENIED", "ABORTED").forEach { result ->
                val completion = decode(
                    encoded(""""result":"$result","reason":"OTHER","message":"Detail""""),
                )
                ...
                val expected = when (result) {
                    ...
                    "DENIED" -> ApprovalCompletion.Denied(completion.clientSoftware, "OTHER", "Detail")
                    else -> ApprovalCompletion.Aborted(completion.clientSoftware, "OTHER", "Detail")
                }
    ...
    fun `only signing protocols reject signatures in approved completions`() {
        ...
        listOf(GitSignProtocol()::decodeCompletion, SshAuthenticationProtocol()::decodeCompletion)
            .forEach { decode ->
                ...
                assertNull(
                    decode(encoded(""""result":"APPROVED","signature":null""")).reason,
                )
            }
    }
