...
@Composable private fun RequestsPreview() = PreviewScreen { RequestsPage(previewRequestSummaries) }
...
@Composable private fun RequestsEmptyPreview() = PreviewScreen { RequestsPage(emptyList()) }

@PreviewTest
@Preview(
    name = "Light",
    group = "requests-offline",
    widthDp = 360,
    heightDp = 800,
    locale = "en",
    uiMode = Configuration.UI_MODE_NIGHT_NO,
...
@Composable
private fun RequestsOfflinePreview() = PreviewScreen {
    RequestsPage(
        previewRequestSummaries,
        "Couldn't connect to the relay. Check your connection and try again.",
    )
}

@Composable
private fun RequestsPage(requests: List<InboxRequestSummary>, problem: String? = null) {
    PreviewNavigation(MainSection.REQUESTS, empty = requests.isEmpty()) { modifier ->
        RequestList(
            onPairClient = ({}).takeIf { requests.isEmpty() },
            requests = requests,
            ...
            onShowSyncProblem = {},
            onOpenSettings = {},
            notificationsEnabled = problem == null,
            onOpen = {},
            onDecision = { _, _ -> },
            modifier = modifier,
        )
...
@Composable private fun InvocationPreview() = PreviewScreen { InvocationPage(previewInvocation) }

@PreviewTest
@Preview(
    name = "Light",
    group = "invocation-ai-review",
    widthDp = 360,
    heightDp = 800,
    locale = "en",
    uiMode = Configuration.UI_MODE_NIGHT_NO,
        ...
        previewInvocation.copy(
            ...
            approvalEvaluation =
                previewEvaluation.copy(
                    secrets =
                        previewEvaluation.secrets.map { it.copy(action = ApprovalAction.ASK_AI) },
                    aiReview =
                        AiReview(
                            AiReviewDecision.ASK_USER,
                            "ANALYZE updates production database statistics. Your instructions require confirmation before making changes.",
                        ),
                ),
        )
    )
...
private fun InvocationFailedPreview() = PreviewScreen {
    InvocationPage(
        previewInvocation.copy(
            ...
            error = "The client confirmation could not be verified.",
        ),
        InboxRequestState.COMPLETED,
    )
}
...
@Composable private fun GitSignPreview() = PreviewScreen { GitSignPage(previewGitSign) }

@Composable
private fun GitSignPage(
    details: GitSignRequestDetails,
    state: InboxRequestState = InboxRequestState.ACTION_REQUIRED,
) {
    GitSignRequestDetail(
        previewRequest(InboxRequestContent.GitSign(details), state),
        {},
        true,
        {},
        {},
...
private val previewSignedRepository =
    GitSignRepository(
...
fun GitSignSignedDarkPreview() = PreviewScreen {
    GitSignPage(
        previewGitSign.copy(
            ...
            decision = ApprovalDecision.APPROVED,
            completionResult = ApprovalCompletionResult.APPROVED,
            repository = previewSignedRepository,
        ),
        InboxRequestState.COMPLETED,
    )
}
...
fun GitSignTagDarkPreview() = PreviewScreen {
    GitSignPage(
        previewGitSign.copy(
            message =
                ("object c81d2f0e6b439a5f70814f07b9a2d3815e64c092\ntype commit\ntag v1.4.0\n" +
            ...
            command = "git",
            arguments = listOf("tag", "-s", "v1.4.0", "-m", "Release 1.4.0"),
...
private fun SshAuthenticationPreview() = PreviewScreen {
    SshAuthenticationPage(previewAuthentication)
}

@Composable
private fun SshAuthenticationPage(
    details: SshAuthenticationRequestDetails,
    state: InboxRequestState = InboxRequestState.ACTION_REQUIRED,
) {
    SshAuthenticationRequestDetail(
        previewRequest(InboxRequestContent.SshAuthentication(details), state),
        {},
        true,
        {},
        {},
...
@Preview(
    name = "Dark",
    group = "ssh-authentication-publickey",
    widthDp = 360,
    heightDp = 800,
    locale = "en",
    uiMode = Configuration.UI_MODE_NIGHT_YES,
...
fun SshAuthenticationPublicKeyDarkPreview() = PreviewScreen {
    SshAuthenticationPage(
        previewAuthentication.copy(
            ...
            username = "git",
            command = "git",
            arguments = listOf("push", "origin", "main"),
...
@Preview(
    name = "Dark",
    group = "ssh-authentication-signed",
    widthDp = 360,
    heightDp = 800,
    locale = "en",
    uiMode = Configuration.UI_MODE_NIGHT_YES,
...
@Preview(
    name = "Dark",
    group = "ssh-authentication-large-text",
    widthDp = 360,
    heightDp = 800,
    fontScale = 1.5f,
    locale = "en",
...
@Composable private fun SecretUploadPreview() = PreviewScreen { UploadPage(previewUpload) }
...
private fun SshUploadPreview() = PreviewScreen {
    UploadPage(
        previewUpload.copy(
            secretType = "ssh",
            uploadedName = "deploy-ssh-staging",
            description = "SSH access to staging application servers.",
            variables = emptyList(),
            variableNames = emptyList(),
            addedVariables = emptyList(),
            publicKey = previewSshKey.publicKey,
            fingerprint = previewSshKey.fingerprint,
        )
    )
}

@Composable
private fun UploadPage(details: SecretUploadRequestDetails) {
    SecretUploadRequestDetail(
        previewRequest(InboxRequestContent.SecretUpload(details)),
        {},
        true,
        {},
        {},
        mapOf("preview-host" to "db.staging.example.com"),
        {},
        { _, _ -> },
        Modifier.fillMaxSize(),
    )
...
private val longSecretNames =
    listOf("orders-production-postgres-password", "orders-production-deployment-token")

@PreviewTest
@Preview(
    name = "Dark",
    group = "requests-long-names",
    widthDp = 360,
    heightDp = 800,
    locale = "en",
    uiMode = Configuration.UI_MODE_NIGHT_YES,
...
fun RequestsLongNamesDarkPreview() = PreviewScreen {
    RequestsPage(
        previewRequestSummaries.map {
            it.copy(
                clientName = longClientName,
                command =
                    if (it.kind == InboxRequestKind.SECRET_USE) multiSecretInvocation.command
                    else it.command,
                arguments =
                    if (it.kind == InboxRequestKind.SECRET_USE)
                        listOf("--environment", "production")
                    else it.arguments,
                secretNames =
                    if (it.kind == InboxRequestKind.SECRET_USE) longSecretNames
...
fun InvocationLongNamesDarkPreview() = PreviewScreen {
    InvocationPage(
        multiSecretInvocation.copy(
            ...
            secretDetails =
                longSecretNames.mapIndexed { index, name ->
                    previewInvocation.secretDetails
                        .single()
                        .copy(
                            name = name,
                            description = "",
                            environmentVariableNames =
                                listOf(if (index == 0) "PGPASSWORD" else "DEPLOY_TOKEN"),
                        )
                },
            approvalEvaluation =
                previewEvaluation.copy(
                    secrets =
                        longSecretNames.mapIndexed { index, name ->
                            previewEvaluation.secrets
                                .single()
                                .copy(secretId = "long-secret-$index", secretName = name)
                        }
                ),
        )
    )
...
private val longCommandArguments =
    listOf(
        "-keystore",
        "/home/maya/.local/share/parcel-android/upload-keystore.p12",
        "-storetype",
        "PKCS12",
        "-storepass:env",
        "KEYSTORE_PASSWORD",
...
fun RequestsLongCommandDarkPreview() = PreviewScreen {
    RequestsPage(
        previewRequestSummaries.map { request ->
            ...
            secretDetails =
                listOf(
                    previewInvocation.secretDetails
                        .single()
                        .copy(
                            name = longCommandSecretName,
                            description = "Google Play upload key passphrase.",
                    ...
                    secrets =
                        listOf(
                            previewEvaluation.secrets
                                .single()
                                .copy(
                                    secretId = "long-command-secret",
                                    secretName = longCommandSecretName,
                                )
                        )
                ),
            workingDirectory = "/home/maya/projects/parcel-android",
            executablePath = longCommandExecutable,
        )
    )
}

private val multiSecretDetails =
    listOf(
        previewInvocation.secretDetails
            .single()
            .copy(
                name = "orders-db-staging",
                description = "PostgreSQL password for staging migrations.",
                environmentVariableNames = listOf("PGPASSWORD"),
            ),
        previewInvocation.secretDetails
            .single()
            .copy(
                name = "deploy-token-staging",
                description = "API token for staging deployments.",
                environmentVariableNames = listOf("DEPLOY_TOKEN"),
            ),
    )
private val multiSecretInvocation =
    previewInvocation.copy(
        command = "./deploy",
        arguments = listOf("--environment", "staging"),
        ...
        approvalEvaluation =
            previewEvaluation.copy(
                secrets =
                    multiSecretDetails.mapIndexed { index, secret ->
                        previewEvaluation.secrets
                            .single()
                            .copy(secretId = "multi-secret-$index", secretName = secret.name)
                    }
            ),
    )
...
@Preview(
    name = "Dark",
    group = "requests-multiple-secrets",
    widthDp = 360,
    heightDp = 800,
    locale = "en",
    uiMode = Configuration.UI_MODE_NIGHT_YES,
...
@Preview(
    name = "Dark",
    group = "invocation-multiple-secrets",
    widthDp = 360,
    heightDp = 800,
    locale = "en",
    uiMode = Configuration.UI_MODE_NIGHT_YES,
