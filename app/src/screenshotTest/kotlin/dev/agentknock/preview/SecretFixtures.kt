...
internal val previewSecret = SecretDetails(
    ...
    instructions = "Allow connection checks and read-only diagnostics. Ask before changing production data.",
...
private val previewPublicKey = SshKeyCodec().publicKey(
    SshKeyAlgorithm.ED25519, ByteArray(32) { (it + 1).toByte() }, "maya@maya-thinkpad",
)
internal val previewSshKey = SshKeyMetadata(
    ...
    comment = "maya@maya-thinkpad", privateKeyAvailable = true,
)
internal val previewSshSecret = previewSecret.copy(
    id = "preview-ssh", name = "work-ssh", description = "Sign work commits and connect to application servers.",
    type = SecretType.SSH, environmentVariables = emptyList(), sshKey = previewSshKey,
    instructions = "Allow signing commits in orders-api. Ask before connecting to production servers.",
)
